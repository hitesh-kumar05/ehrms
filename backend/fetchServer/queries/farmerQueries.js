...
exports.getFarmerBasicDetail_Base_QueryParamObj = function (serachColumn, searchValue1, searchValue2) {
    ...
    TRIM(mf.farmer_name_hi) as farmer_name_hi, mf.relation, mf.father_name, IF(mf.dob = '1968-01-01', NULL, mf.dob) as dob, 
    mf.subcaste_code,mc.caste_code ,mf.aadhar_verification,mf.gender, IF(mf.mobile_no = '9876543210', '', mf.mobile_no) as mobile_no,
    ...
    fr.Name as nominee_name, fr.TypeofPerson AS nominee_type, fr.aadhar_ref AS nominee_aadhar_ref,fr.Relation AS nominee_relation, fr.AadharNo AS nominee_aadhar,  mbb.bank_name, 
...
exports.checkVillageKhasra_Base_QueryParamObj = function (village_code, khasra_no, fs_id) {
    ...
    AND (ld.carry_forward_status IS null) ${where_clause}`;
...
exports.getFarmerBasicDetailForReciptQueryParamObj = function (fs_id) {
    ...
    mc.caste_code as category, mf.subcaste_code AS subcaste_code,
       ...
       mbb.bank_code as bank_code, dd.District_ID AS bank_district, CAST(fs.branch_code AS INT) AS branch_code, CAST(mf.village_code AS INT) AS village_code, 
        ...
        fs.entry_type_code,IFNULL(fs.updated_dtstamp, fs.dtstamp) AS dtstamp, fs.is_update_society, fs.is_update_tehsil, d.villcdname, d.tehcdname, d.cdname, d.halka, mc.caste_name,
        mf.aadhar_verification, fs.pfms_flag
       FROM mas_farmer mf
       INNER JOIN farmer_society fs ON fs.uf_id = mf.uf_id
       ...
       INNER JOIN mas_relation mr ON fr.Relation = mr.rel_id
        WHERE fs.fs_id = ? AND fs.operation_id NOT IN (3, 5);`, params: [fs_id]
    })
}
...
exports.getFarmerCropByVillageCodeAndKhasraForReciept = function (fs_id, village_code, khasra_no, land_table) {
    ...
    let q = `SELECT cd.crop_area, mc.crop_name_hi, FORMAT(CAST(cd.crop_area AS FLOAT), 4) crop_area_d, cd.crop_status_code, mcs.crop_status_name, cd.crop_code
    FROM ${table_name} cd 
    INNER JOIN mas_crop mc ON mc.crop_code = cd.crop_code
    INNER JOIN mas_crop_status mcs ON mcs.crop_status_code = cd.crop_status_code
...
exports.getFarmerBasicDetailsForVarisanQueryParam = function (fs_id) {
    ...
    fs.account_no, mf.uf_id, fs.fs_id, CAST(dd.District_ID AS SIGNED)  AS bank_district, fs.branch_code, 
	 mf.village_code, fs.society_id, 
        ...
        CONCAT('XXXX-XXXX-', SUBSTRING(mf.aadhar_number, 9,LENGTH(mf.aadhar_number))) AS aadhar_no, fs.dtstamp
        FROM mas_farmer mf 
        INNER JOIN farmer_society fs ON fs.uf_id = mf.uf_id
        ...
        WHERE fs.fs_id = ? AND fs.operation_id NOT IN (3, 5)`,
        p = [fs_id];
    return { "query": q, "params": p };
}
...
INNER JOIN farmerrepresentative fr ON fr.fs_id = fs.fs_id
INNER JOIN mas_relation mr ON fr.Relation = mr.rel_id
WHERE fs.operation_id NOT IN (3, 5) AND ${whereColumn};`
...
exports.getFarmerRgLandQueryParamObj = function (serachColumn, searchValue1, searchValue2) {
    ...
    ld.OwnerName, ld.owner_type_code, ld.flag_varis_land, mot.owner_type_name, ld.land_area, FORMAT(CAST(ld.land_area AS FLOAT), 4) land_area_d, ld.land_type_code, mlt.land_type_name_hi , 1 as 'land_table'
        FROM land_details ld 
        INNER JOIN mas_farmer mf ON ld.uf_id = mf.uf_id
        inner JOIN mas_villages d ON d.vsr_census = ld.village_code
        INNER JOIN mas_land_type mlt ON mlt.land_type_code = ld.land_type_code
...
exports.getFarmerRgCropDetailsByVillageKhasraQueryParamObj = function (village_code, khasra_no) {
    ...
    INNER JOIN mas_crop mc ON mc.crop_code = cd.crop_code
    INNER JOIN mas_crop_status mcs ON mcs.crop_status_code = cd.crop_status_code
    WHERE cd.village_code = ? and cd.khasra_no = "${khasra_no}"; `,
        params: [village_code]
    })
}

exports.getLandData_New_QueryParamObj = (village_code, khasra_str) => {
    return ({
...
exports.getLandData_Base_QueryParamObj = (village_code, khasra_str) => {
    return ({
        query: `SELECT ld.village_code,ld.khasra_no, ld.OwnerName, mv.villcdname,ld.land_area
 FROM land_details ld
 INNER JOIN mas_villages mv ON mv.vsr_census = ld.village_code
...
exports.getCropData_Base_QueryParamObj = (village_code, khasra_str) => {
    return ({
...
exports.getCropData_New_QueryParamObj = (village_code, khasra_str) => {
    return ({
        query: `SELECT cd.village_code,cd.khasra_no, cd.crop_code,mc.crop_name,cd.crop_area
 FROM crop_details cd
 INNER JOIN mas_crop mc ON mc.crop_code = cd.crop_code
...
exports.getFarmerBasicDetailsForRejectionFarmerQueryParam = function (fs_id, isNewFarmer) {
    let where_clause = ``;
    if (isNewFarmer == 'Y') {
        where_clause += ` AND fs.operation_id = 4 `
    }
    else {
        where_clause += ` AND fs.delete_status = 'R' `
    }

    let q = `SELECT CONCAT('XXXXXXXX', SUBSTRING(mf.aadhar_number, 9)) aadhar_number, mf.farmer_name_aadhar, mf.farmer_name_hi, case when mf.relation = 'F' then 'पिता' when mf.relation = 'M' then 'माता' when mf.relation = 'H' then 'पति' END relation, mf.father_name, mf.dob, 
    mc.caste_code as category, mf.subcaste_code AS subcaste_code,
        mf.gender, CAST(mf.mobile_no AS CHAR) mobile_no, fs.membership_no, mf.pincode, mf.address, fs.account_no, fs.account_no AS caccount_no, mf.uf_id, fs.fs_id, 
       mbb.bank_code as bank_code, dd.District_ID AS bank_district, CAST(fs.branch_code AS INT) AS branch_code, CAST(mf.village_code AS INT) AS village_code, 
        mt.District_ID AS district, mt.Tehsil_ID AS tehsil_id, mbb.ifsc_code,
        ...
        case when fr.TypeofPerson = 1 then 'रिस्तेदार' when fr.TypeofPerson = 2 then 'विश्वसनीय व्यक्ति' when fr.TypeofPerson = 0 then 'स्वयं' end nominee_type_name,
        fr.Name AS nominee_name, mr.rel_name AS nominee_relation, CONCAT('XXXXXXXX', SUBSTRING(fr.AadharNo, 9)) AS nominee_aadhar, fs.farmer_code,
        fs.entry_type_code, fs.dtstamp, d.villcdname, d.tehcdname, d.cdname, d.halka, mc.caste_name, mc.subcaste_name,
        mf.aadhar_verification, fs.pfms_flag
       FROM mas_farmer mf
       INNER JOIN farmer_society fs ON fs.uf_id = mf.uf_id
       INNER JOIN mas_villages d ON d.vsr_census = mf.village_code
       ...
       INNER JOIN farmerrepresentative fr ON fr.fs_id = fs.fs_id
       INNER JOIN mas_relation mr ON fr.Relation = mr.rel_id
        WHERE fs.fs_id = ? ${where_clause}`,
        p = [fs_id];
    return { "query": q, "params": p };
}

exports.getAadharDetailsForSanshodhanByFsidQandP = function (fs_id) {
    return ({
        ...
        fs.farmer_code, mf.farmer_name_hi,
                mf.father_name, mf.mobile_no, s.Society_Name, mv.villcdname, mv.halka, mv.tehcdname, mv.cdname, 
                  mf.gender, mf.subcaste_code, mc.caste_code, mf.address, mf.mobile_no, mf.dob, fs.village_code ,
                  case when mf.relation = 'F' then 'पिता' when mf.relation = 'M' then 'माता' when mf.relation = 'H' then 'पति' END relation_hi,
                  mf.relation
        ...
        INNER JOIN mas_caste mc ON mc.subcaste_code = mf.subcaste_code
        WHERE fs.fs_id = ? AND fs.operation_id NOT IN (3, 5) AND rsv.edit_type_adhar_status = 1`, params: [+fs_id]
    })
}

exports.getBasicDetailsForSanshodhanByFsidQandP = function (fs_id, type) {
    ...
    mc.caste_code, mf.subcaste_code,
       ...
       CAST(mf.village_code AS INT) AS village_code, 
        ...
        fs.entry_type_code, IFNULL(fs.updated_dtstamp, fs.dtstamp) AS dtstamp, d.villcdname, d.tehcdname, d.cdname, d.halka, d.VillageID AS village_id, 
		  mc.caste_name, mc.subcaste_name, mv.villcdname AS 'society_village', fs.village_code AS society_village_code, fs.pfms_flag,
		  mv.VillageID AS 'society_village_id', case when mf.relation = 'F' then 'पिता' when mf.relation = 'M' then 'माता' when mf.relation = 'H' then 'पति' END relation_hi,
          mf.relation
       FROM mas_farmer mf
       INNER JOIN farmer_society fs ON fs.uf_id = mf.uf_id
       INNER JOIN mas_villages d ON d.vsr_census = mf.village_code
       ...
       INNER JOIN mas_villages mv ON mv.vsr_census = fs.village_code
       INNER JOIN raeo_sanshodhan_verification rsv ON fs.fs_id = rsv.fs_id
        WHERE fs.fs_id = ? AND fs.operation_id NOT IN (3, 5) AND ${type == 2 ? `edit_type_basicDetails_status` : `edit_type_account_status`} = 1`;

    let p = [fs_id];
    return ({ query: q, params: p });
}

exports.getTrustBasicDetail_Base_QueryParamObj = function (farmer_code) {
    let q = `SELECT TRIM(f.FarmerName) AS farmer_name_hi, f.MobileNo AS mobile_no, f.Address AS society_address,
f.FarmerCode AS farmer_code, CAST(md.District_ID AS SIGNED) AS district, mt.Tehsil_ID AS tehsil_id,
CAST(f.NewVLocationCode AS SIGNED) AS society_village_code
FROM farmer_reg_trust_only f
INNER JOIN mas_villages mv ON mv.vsr_census = f.NewVLocationCode
INNER JOIN mas_districts md ON md.Rev_district_id = mv.distno
INNER JOIN mas_tehsil mt ON mt.Rev_dist_id = mv.distno AND mt.Rev_teh_id = mv.tehsilno
WHERE f.FarmerCode = '${farmer_code}'; `;
    let p = [];
    return ({ query: q, params: p });
}

exports.getTrustLandDetails_Base_QueryParamObj = function(farmer_code){
    let q = `SELECT d.villcdname, d.VillageID, ld.FarmerCode as farmer_code, ld.VLocationCode as village_code,
ld.KhasraNo as khasra_no, ld.Transferred as OwnerName, ld.OwnerType as owner_type_name, ld.RakbaOfPaddy as land_area,
1 as land_type_code, 'राजस्व ग्राम' as land_type_name_hi , 'P' as is_verified
    FROM farmerregkhasradetails_trust_only ld
    LEFT JOIN mas_villages d ON d.vsr_census = ld.VLocationCode
    WHERE ld.FarmerCode = '${farmer_code}';`;

    let p = [];
    return ({ query: q, params: p });
}

exports.getFarmerBasicDetails_Base_AadharNullQPObj = function(farmer_code){
    let q = `SELECT CAST(fs.AadharNo AS CHAR) as aadhar_number, fs.AadharName as farmer_name_aadhar, fs.FarmerAgrID as uf_id, 
        fs.FarmerCode as farmer_code, fs.FarmerName as farmer_name_hi,
                fs.Father_HusbandName as father_name, fs.MobileNo as mobile_no, s.Society_Name, mv.villcdname, mv.halka, mv.tehcdname, mv.cdname, 
                  fs.Sex as gender, fs.SubCaste as subcaste_code, mc.caste_code, fs.Address as address, fs.DOB as dob, fs.NewVLocationCode as village_code,
			fs.Relation as relation_hi, fs.Relation as relation, fs.society_id
         FROM farmer_reg_aadhar_null fs
         INNER JOIN society s ON s.society_id = fs.society_id
        INNER JOIN mas_villages mv ON mv.vsr_census = fs.NewVLocationCode
        INNER JOIN mas_caste mc ON mc.subcaste_code = fs.SubCaste
        WHERE fs.FarmerCode = '${farmer_code}' AND fs.AadharNo IS null; `;
    let p = [];
    return ({ query: q, params: p });
}